...
function clientCheckHandler (shepherd, req, rsp) {
    ...
    if (cnode) {
        if (chkAttr.sleep) {
            ...
            cnode.sleepCheck(chkAttr.duration);
        } else {
            cnode.sleepCheck();
            cnode._setStatus('online');
        }
...
function clientLookupHandler (shepherd, req, rsp) {
    var lookupType = cutils.pathSlashParser(req.url)[1],
        clientName = buildDevAttr(req).clientName,
...
function buildChkAttr(req) {
    ...
    _.forEach(queryParams, function(queryParam) {     
        if(queryParam[0] === 'chk') {
            if (queryParam[1] === 'on')
                chkAttr.sleep = false;
            else if (queryParam[1] === 'off')
                chkAttr.sleep = true;
        } else if (queryParam[0] === 't') {
            chkAttr.duration = Number(queryParam[1]);
        } else {
            invalidAttrs.push(queryParam[0]);
        }
    });

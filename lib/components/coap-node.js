...
CoapNode.prototype.writeReq = function (path, value, callback) {
    ...
    if (chkErr) {
        ...
        reqObj.options = _.isPlainObject(value) ? {'Content-Format': 'application/json'} : {'Content-Format': 'text/plain'};
...
CoapNode.prototype._reqCheck = function (type, path, data) {
    ...
    if (!_.isString(path))
        throw TypeError('path should be a string.');
    ...
    switch (type) {
        ...
        case 'writeAttrs':
            if (!_.isPlainObject(data))
                throw TypeError('attr should be a object.');

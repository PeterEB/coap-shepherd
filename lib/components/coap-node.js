    ...
    if (chkErr) {
        ...
        this.shepherd.request(reqObj).then(function (rsp) {
            ...
            if (rsp.code === RSP.content) {  // only 2.05 is with data
                rspObj.data = cutils.checkRescType(path, rsp.payload);
    ...
    if (chkErr && path !== '/heartbeat') {
        ...
        this.shepherd.request(reqObj).done(function (observeStream) {
            ...
            if (observeStream.code === RSP.content) {
                ...
                rspObj.data = cutils.checkRescType(path, observeStream.payload);

                if (path !== '/heartbeat')
                    self.observedList.push(cutils.getSoKeyPath(reqObj.pathname));
    ...
    if (chkErr) {
        ...
        this.shepherd.request(reqObj).done(function (rsp) {
            ...
            if (rsp.code === RSP.content) 
                self._cancelObserver(cutils.getSoKeyPath(reqObj.pathname));
...
CoapNode.prototype._dumpSummary = function () {
    var self = this,
        dumped = {},
        includedKeys = [ 'clientName', 'clientId', 'lifetime', 'version', 'ip', 'mac', 'port', 'objList', 'observedList' ];
...
CoapNode.prototype._updateSoAndDb = function (path, data, callback) {
        var deferred = Q.defer(),
        self = this,
        dataType = cutils.pathDataType(path),
        dataObj = cutils.getSoKeyObj(path),
        iObjsUpdater = [],
        iidArray = [];

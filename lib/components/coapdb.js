...
Coapdb.prototype.findByClientNameWithId = function (cName, callback) {
    var deferred = Q.defer();
    ...
    if (path === 'clientName' || path === 'locationPath') {
        ...
        this.findByClientName(cName).then(function (doc) {
            if (!doc) {
                ...
                self.db.update({ clientName: cName }, { $set: objToReplace }, {}, function (err, numRemoved) {
                    if (err)
                        deferred.reject(err);
                    else
                        deferred.resolve(numRemoved);
    ...
    this.findByClientName(cName).then(function (doc) {
        if (!doc) {
            ...
            if (invalidPath.length !== 0) {
                ...
                self.db.update({ clientName: cName }, { $set: objToUpdate }, { multi : true }, function (err, numReplaced) {
                    if (err) {
                        deferred.reject(err);
                    } else {

...
CoapNode.prototype.dump = function () {
    var self = this,
        excludedKeys = [ 'shepherd', 'defaultPort', 'status', '_registered', '_streamObservers', 'lifeChecker', '_heartbeat'],
        dumped = {};
...
CoapNode.prototype._reqCheck = function (type, path, data) {
    ...
    switch (type) {
        ...
        case 'write':
            if (!_.isString(path))
                throw new TypeError('path should be a string.');
            else if (!cutils.uriParser(path)[1])
                throw new Error('path should contain Object ID, Object Instance ID.');
...
function notifyHandler (cnode, reqObj, value, type) {
    ...
    if (shepherd._enabled && cutils.uriParser(reqObj.pathname)[0] === 'heartbeat') {

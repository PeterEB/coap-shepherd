...
CoapShepherd.prototype.removeNode = function (clientName, callback) {   // [TODO] need callback?
    var deferred = Q.defer(),
        shepherd = this,
...
// [FIXME] you should decrease the count?
            ...
            if (shepherd._shepherdTest)
                shepherd.emit('ind', { type: 'deregistered', msg: clientName });
...
function _coapServerStart(shepherd, callback) {
    ...
    server.listen(shepherd.port, function (err) {
        if (err)
            deferred.reject(err);
        else
    ...
    if (!cnode) {
        ...
        if (shepherd._shepherdTest) {
            cnode._readAllResource().then(function (msg) {
                ...
                rsp.end('');
                shepherd.emit('ind', { type: 'registered', msg: cnode });  
            }).fail(function (err) {
...
function _clientUpdateHandler (shepherd, req, rsp) {
    ...
    if (cnode) {
        cnode._updateAttrs(devAttr).then(function (diffAttrs) {
            ...
            if (shepherd._shepherdTest) 
                shepherd.emit('ind', { type: 'update', msg: msg });
        }, function (err) {
            rsp.code = RSP.dberror;
            rsp.end('');
            shepherd.emit('error', err);
...
function _clientDeregisterHandler (shepherd, req, rsp) {
    ...
    if (cnode) {
        shepherd.removeNode(clientName).then(function () {
...
// [TODO] check pathname & lookupType
    if (cnode) {
        ...
        shepherd.emit('ind', { type: 'lookup' , msg: clientName });
        deferred.resolve();
    } else {
        rsp.code = RSP.notfound;
        rsp.end('');
...
// client register
    testReqOdj.hostname = shepherd.ip;
    testReqOdj.port = shepherd.port;

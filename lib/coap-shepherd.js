...
var RSP = { ok: '2.00', created: '2.01', deleted: '2.02', changed: '2.04', content: '2.05', badreq: '4.00',
            notfound: '4.04', notallowed: '4.05', timeout: '4.08',  dberror: '5.00' };
...
CoapShepherd.prototype.deregisterNode = function (clientName, callback) {
    ...
    if (cnode) {
        ...
        deferred.resolve({ status: RSP.deleted });
    } else {
        deferred.resolve({ status: RSP.notfound });
    }
...
function _clientRegisterHandler (shepherd, req, rsp) {
    var deferred = Q.defer(),
        devAttr = cutils.buildDevAttr(req),
...
function _clientUpdateHandler (shepherd, req, rsp) {
    var deferred = Q.defer(),
        ...
        ipChk = _clientReqIpChk(cnode, req),
        diff,
        chkErr;
...
// server read
        ...
        if (msg.status === RSP.content && msg.data === 'test') {
// client deregister

...
function _coapRequest(reqObj) {
    ...
    reqChecker = setTimeout(function () {
        ...
        // It would be good to let rsp.code = '4.08' (Timeout) for your users to get the status code -
...
function _clientUpdateHandler (shepherd, req, rsp) {
    ...
    shepherd._findByClientId(clientId).then(function (dev) {
        ...
        if (shepherd._shepherdTest) {
            ...
            return node._readAllResource().then(function () {
                return node.dbSave();
            });
        }
    }).fail(function (err) {
...
function _testShepherd (shepherd) {
    ...
    shepherd.request(testReqOdj).then(function (rsp) {  //register
        ...
        return shepherd.request(testReqOdj);            //update
    }).then(function (rsp) {
        if (rsp.code === '2.04') {
            testNode = shepherd.find('shepherdTest');
            return testNode.read('/');                  //read
        } else {

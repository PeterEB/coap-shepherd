...
CoapShepherd.prototype.request = function (reqObj, callback) {
    ...
    if (this._server)
        socket = this._server._sock;

    if (!this._enabled) {
        deferred.reject(new Error('server does not enabled.'));
    } else {
        if (!_.isNil(reqObj.payload) && reqObj.options && reqObj.options['Content-Format'] === 'application/json') 
        ...
        coapRequest(reqObj, socket).done(deferred.resolve, deferred.reject);
    }
 ...
 * Private function                                      *
 *********************************************************/
function coapRequest(reqObj, socket) {
    var deferred = Q.defer(),
        agent = new coap.Agent({ type: config.connectionType, socket: socket }),
        req = agent.request(reqObj);
    ...
    if (enabled) {
        shepherd._hbChecker = setInterval(function () {
            _.forEach(shepherd._registry, function (cn) {
                ...
                if (cn.status === 'online' && cn._heartbeatEnabled && ((now - cn._heartbeat) > hbTimeout)) {

...
function _clientReqHandler(shepherd, req, rsp) {
    ...
    switch (optType) {
        ...
        case 'update':
            ...
            break;
        case 'heartbeat':
            // reqHdlr = _clientHeartbeatHandler;
            break;
        case 'deregister':
...
//  [TODO]
// function _clientHeartbeatHandler (shepherd, req, rsp) {
//     var devAttr = cutils.buildDevAttr(req),
//         clientId = cutils.uriParser(req.url)[1],
//         cnode = shepherd._findByClientId(clientId);

//     function heartbeatHandler(cnode, value) {
//         cnode._heartbeat = Number(value.toString());
//     }

//     if (cnode) {
//         if (rsp.code === '2.04') {
//             cnode._heartbeat = rsp.payload;
//             cnode._streamObservers.heartbeat = rsp;

//             rsp.on('data', function (value) {
//                 rsp.removeAllListeners('data');
//                 rsp.on('data', function (value) {
//                     heartbeatHandler(cnode, value);
//                 });
//             });
//         }
...
function _clientReqParser (req) {
    ...
    switch (req.method) {
        ...
        case 'PUT':
            // optType = 'update';
            optType = (req.headers.Observe === 0) ? 'heartbeat' : 'update' ;
            break;
        case 'DELETE':
            optType = 'deregister';
            break;

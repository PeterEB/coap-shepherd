...
CoapShepherd.prototype.request = function (reqObj, callback) {
    ...
    if (!this._enabled) {
        ...
        coapRequest(reqObj, agent).done(deferred.resolve, deferred.reject);
    }
 ...
 * Private function                                      *
 *********************************************************/
function coapRequest(reqObj, originalAgent) {
    var deferred = Q.defer(),
        agent = reqObj.observe ? new coap.Agent({ type: config.connectionType }) : originalAgent,
        req = agent.request(reqObj);
